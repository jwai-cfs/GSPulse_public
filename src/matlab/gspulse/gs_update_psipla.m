...
for i = 1:N
  ...
  H = zeros(2,2);
  R = rgg(:);
  H(1,:) = [R'*b.pprimezr  1./(mu0 * R') * b.ttprimezr] * dA;  % eqn A.39
  H(2,1) = 3*pi*R'*b.preszr*dA;                                % eqn A.40 
  c = H \ [ref.Ip; ref.Wk];

  % find the plasma current distribution from Grad-Shafranov eqn A.2
