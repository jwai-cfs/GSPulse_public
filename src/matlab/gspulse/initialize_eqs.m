...
eqs1N = {};
for i = 1:length(t1N)  
  ref = structts2struct(shapes, {'cp_r', 'cp_z'}, t1N(i));
  out = ~inpolygon(rgg, zgg, ref.cp_r, ref.cp_z);

  eq.psipla = psipla1N(:,i);
  eq.psizr = reshape(psizr1N(:,i), tok.nz, tok.nr);
  eq.outside_plasma_domain_mask = out;
  eq.psibry = mean(bicubicHermite(tok.rg, tok.zg, eq.psizr, ref.cp_r, ref.cp_z));
  eqs1N{i} = eq;
end
