...
%
% Additional info: 
%   Appendix A.2 of the gspulse_algorithm.pdf describes optimization across
...
init.t1 = t1;
init.t2 = t2;
init.u0 = structts2vec(s.mpcsoln, cv.u_name, t0);
init.u1 = structts2vec(s.mpcsoln, cv.u_name, t1);
init.x1 = structts2vec(s.mpcsoln, cv.x_names, t1);
init.x2 = structts2vec(s.mpcsoln, cv.x_names, t2);
init.e1 = structts2vec(s.mpcsoln, cv.e_names, t1);
init.e2 = structts2vec(s.mpcsoln, cv.e_names, t2);
